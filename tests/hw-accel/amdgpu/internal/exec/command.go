...
func (p *PodCommand) ExecuteAndCleanup(timeout time.Duration) (string, error) {
	...
	case err := <-readyCh:
		if err == nil {
			klog.Infof("Pod %s became Ready", p.name)
		} else {
			klog.V(90).Infof("Pod %s not Ready (expected for short-lived job): %v", p.name, err)
		}
	...
	case err := <-doneCh:
		if err == nil {
			klog.Infof("Pod %s completed successfully", p.name)
		} else {
			klog.Warningf("Pod %s completion check failed: %v", p.name, err)
		}
	...
	if waitErr != nil {
		klog.Errorf("Pod %s did not complete successfully: %v", p.name, waitErr)

		klog.V(90).Infof("Fetching logs for failed pod %s", p.name)
	}

...
func WaitForAMDGPUNodes(apiClient *clients.Settings, timeout time.Duration) error {
	klog.V(90).Info("Waiting for nodes to be labeled with AMD GPU features")
	...
	for {
		...
		default:
			...
			if len(nodes.Items) > 0 {
				klog.V(90).Infof("Found %d nodes with label %s=%s", len(nodes.Items), labelKey, labelValue)

				return nil
			}

			klog.V(90).Infof("No nodes found with label %s=%s, waiting...", labelKey, labelValue)
			time.Sleep(10 * time.Second)
		}
	}
}
...
func VerifyAMDGPUKernelModule(apiClient *clients.Settings) error {
	klog.V(amdgpuparams.AMDGPULogLevel).Infof("Verifying amdgpu kernel module is blacklisted on nodes")
	...
	if len(nodes.Items) == 0 {
		klog.V(amdgpuparams.AMDGPULogLevel).Infof("No nodes with AMD GPU labels found - verification skipped")
...
func checkKernelModuleOnNode(apiClient *clients.Settings, nodeName string) bool {
	klog.V(amdgpuparams.AMDGPULogLevel).Infof("Checking amdgpu module status on node %s", nodeName)
...
func logModuleLoadStatus(nodeName, output string) {
	if strings.Contains(output, "amdgpu") && !strings.Contains(output, "MODULE_NOT_LOADED") {
		klog.V(amdgpuparams.AMDGPULogLevel).Infof("WARNING: amdgpu module is still loaded on node %s", nodeName)
		klog.V(amdgpuparams.AMDGPULogLevel).Infof("Module status: %s", output)
	} else {
		klog.V(amdgpuparams.AMDGPULogLevel).Infof("Good: amdgpu module is not loaded on node %s", nodeName)
	}
}
...
func execCommandOnNode(
	...
	klog.V(amdgpuparams.AMDGPULogLevel).Infof("Executing command on node %s: %s", nodeName, command)
	...
	klog.V(amdgpuparams.AMDGPULogLevel).Infof("Command executed successfully on node %s", nodeName)
